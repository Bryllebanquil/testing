...
class TestRegistrySystemCompatibility(unittest.TestCase):
    ...
    def test_backup_functionality(self):
        ...
        # Test backup with mock registry
        with patch('enhanced_registry.winreg') as mock_winreg:
            mock_key = MagicMock()
            ...
            mock_winreg.QueryValueEx.return_value = (1, 4)  # value, type
            
            # This would normally be called internally
            backup_data = self.enhanced_registry._backup_registry_keys([
                (mock_winreg.HKEY_LOCAL_MACHINE, r"TEST\PATH", "TEST_VALUE", 1)
            ])
            
            self.assertIsInstance(backup_data, list)
    
    def test_cleanup_retry_logic(self):
        """Test cleanup retry logic"""
        with patch('enhanced_registry.winreg') as mock_winreg:
            # Simulate failure then success
            call_count = 0
            
            def delete_key_side_effect(*args):
                nonlocal call_count
                call_count += 1
                if call_count == 1:
            ...
            mock_winreg.DeleteKey.side_effect = delete_key_side_effect
